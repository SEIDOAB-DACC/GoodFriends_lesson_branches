...
**Purpose:** after a create/update writes to the database, the caller expects the fully populated, up-to-date item back rather than the raw DTO it sent in. A reusable single-item read gives `Create`/`Update` (and later a `ReadItem` controller action) one consistent way to fetch that.

`PetsDbRepos` currently only has the paged `ReadPetsAsync`. Add a single-item version, mirroring `ReadFriendAsync`:
...
## Step 4 — Add `CreatePetAsync` to `PetsDbRepos`

**Purpose:** this is the actual "C" in CRUD — turning a validated DTO into a new row. The guard clause enforces that clients can't dictate an existing id, and — because the owner FK is required — the create must also validate the referenced `Friend` actually exists before saving (otherwise EF Core will throw a less helpful DB-level error).
...
public async Task<ResponseItemDto<IPet>> CreatePetAsync(PetCuDto itemDto)
{
    ...
    // 3. Resolve and assign the owning Friend (Step 6 helper) — required, so missing/invalid FriendId must throw
...
## Step 5 — Add `UpdatePetAsync` to `PetsDbRepos`
...
public async Task<ResponseItemDto<IPet>> UpdatePetAsync(PetCuDto itemDto)
{
    ...
    // 4. reassign the owning Friend if FriendId changed (Step 6 helper)
...
**Purpose:** resolves the flat `FriendId` into an actual tracked `FriendDbM` EF Core can wire up as the owning relationship. Because the FK is required, this helper has no "set to null" branch like Address's — a missing or invalid `FriendId` must always throw.
...
Call this helper from both `CreatePetAsync` and `UpdatePetAsync`, just like Friends does for its navigation properties.

## Step 7 — Extend `IPetsService` and `PetsServiceDb`

**Purpose:** the controller depends on the `IPetsService` abstraction, not on `PetsDbRepos` directly, so the new repo methods must be exposed through the service interface and its implementation before a controller can call them.
...
`Services/PetsServiceDb.cs` — thin 1:1 pass-throughs, matching the existing style:
```csharp
...
## Step 8 — Extend `PetsController`

**Purpose:** exposes the new service methods as HTTP endpoints — `POST` for create and `PUT {id}` for update — and adds the `ReadItem`/`ReadItemDto` actions needed to fetch a single pet (and its CU-DTO shape) for testing.
